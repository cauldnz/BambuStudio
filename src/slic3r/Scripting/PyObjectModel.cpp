...
#include "libslic3r/CutUtils.hpp"       // object.cut_with_connectors (Slic3r::Cut)
                          ...
                          EnforcerBlockerType state, FacetsAnnotation ModelVolume::*member)
{
    ...
    TriangleSelector selector(mesh);
    if (!facets.get_data().first.empty())
...
void register_object_model(py::module_ &m)
{
    ...
    py::class_<PyObject>(m, "Object")
        ...
        .def("duplicate", [](const PyObject &o) {
            ...
            const double gap = src->bounding_box().size().x() + 5.0;
        ...
        .def("cut_with_connectors", [](const PyObject &o, double z, py::iterable connectors,
            ...
            for (py::handle item : connectors) {
                ...
                nv->cut_info = ModelVolume::CutInfo(CutConnectorType::Plug, (float) radius, (float) depth, 0.0f, 0.1f);
        ...
        // (applied for the "modifier" type). `cover` sizes+places a box over a
        // region: "zband:a-b" (heights above the object base) or "bbox"/"all".
        ...
        .def("add_modifier", [](const PyObject &o, py::object shape, py::object path,
            ...
            if (!path.is_none()) {
                ...
                Vec3d c = mb.center();
                mesh.translate(-c.x(), -c.y(), -c.z());   // center the STL at origin
                ...
                BoundingBoxf3 mb = mesh.bounding_box();
                Vec3d c = mb.center();
                mesh.translate(-c.x(), -c.y(), -c.z());   // center the unit mesh at origin
                if (!cover.is_none()) {
                    ...
                    } else {   // cylinder / cone: size=[radius, height]
        ...
        .def("parts", [](const PyObject &o) {
            ...
            for (size_t i = 0; i < obj->volumes.size(); ++i) {
                ...
                d["is_negative"] = v->is_negative_volume();
                py::dict st;
        ...
        .def("adaptive_layer_height", [](const PyObject &o, double quality) {
            ...
            SlicingParameters params = PrintObject::slicing_parameters(cfg, *obj, (float) maxz);
            std::vector<double> prof = layer_height_profile_adaptive(params, *obj, (float) quality);
            { bool _reset = false; PrintObject::update_layer_height_profile(*obj, params, prof, _reset); }
